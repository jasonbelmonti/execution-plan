...
| EP-ACT-2 | EP-PRE-2 | EP-OUT-1, EP-OUT-2 | test/prune.test.ts, prune fixtures | Invoke the actual CLI over isolated copies of the same candidate fixture; assert exact dry-run paths against independently specified expected entries, an unchanged cache snapshot, and normal-mode deletion. | Tests prove exact dry-run reporting and immutability while preserving normal pruning over identical selection inputs. | `git diff -- test/prune.test.ts` captured in the implementation handoff. | EP-RESP-1 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| EP-GATE-1 | EP-OUT-1 | Run `npm run typecheck`. | The command exits zero with the new option and conditional mutation branch typed; this supporting gate does not prove dry-run behavior. | Run `mkdir -p .codefactory/execution-plans/cache-prune-dry-run/evidence`; redirect combined output to the named evidence file; append the exit status, timestamp, and input-state record described below before evaluating success. | `.codefactory/execution-plans/cache-prune-dry-run/evidence/EP-GATE-1-typecheck.txt` | Verify the file is nonempty, contains no type error, records exit status 0, and identifies unchanged source inputs across the check. Reassess applicability after later changes using the procedure below. | EP-RESP-1 |
| EP-GATE-2 | EP-OUT-1, EP-OUT-2 | Run `npm test -- --runInBand test/prune.test.ts`, then `npm test`. | Dry-run reports exactly the expected eligible entries without filesystem changes; normal mode deletes them; all existing tests pass. Returning no entries or deleting in dry-run must fail these assertions. | Confirm the evidence directory exists; redirect each command's combined output to its named evidence file; append each exit status, timestamp, and input-state record described below before evaluating success. | `.codefactory/execution-plans/cache-prune-dry-run/evidence/EP-GATE-2-focused.txt` and `.codefactory/execution-plans/cache-prune-dry-run/evidence/EP-GATE-2-regression.txt` | Verify both files contain passing behavioral summaries plus exit status 0 and identify unchanged source inputs across each check. Confirm required CLI cases ran; helper-only or skipped cases do not prove the outcome. Reassess applicability before handoff. | EP-RESP-1 |

For each gate report, record `git rev-parse HEAD`, `git diff --binary`, and `git diff --cached --binary` alongside `node --version`, `npm --version`, the command, and its exit status. Record content fingerprints for the consumed source, tests, fixtures, package manifest and lockfile, and typecheck/test configuration, including relevant untracked or ignored generated inputs. Compare those source-input fingerprints before and after each command; unexpected changes prevent admission. Treat the disposable cache copies as runtime state whose intended mutations are verified by assertions, not as source fixtures to keep unchanged. Keep the reports themselves outside the input set to avoid self-invalidating captures.

Before reusing a gate result, compare its recorded inputs with current state. EP-ACT-2 changes test inputs, so reassess EP-GATE-1: rerun typechecking if its inputs include those tests; otherwise record the inspected exclusion and retain its result. A later change to selectCandidates, CLI wiring, assertions, fixtures, dependencies, or configuration invalidates the affected EP-GATE-2 results. Preserve old reports as historical evidence and rerun the affected checks before handoff. An unrelated document edit does not require replay when it is outside those inputs. Record these observations in the implementation handoff, not as completed work in this plan; if the route must change, revise it under the normal change-control rules.
