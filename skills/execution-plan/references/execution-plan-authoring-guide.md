...
- Treat the single `Plan Control` row as the sole authority for lifecycle state and planning depth; do not duplicate them in frontmatter. The structural profile rejects `status` and `planning_depth` frontmatter fields.
...
The structural profile checks Markdown-native metadata—exact `ExecutionPlan` type, required non-blank values, exact artifact version, reserved `okf_version` and duplicate lifecycle-field exclusions—along with required section order, unresolved placeholders, exact table schemas, total table cardinality, documented per-schema row bounds through Markdown Engine 3.5.0, and generic action, gate, phase, and precondition ID coverage through `tableColumnCoverage`. The execution-plan validator consumes Markdown Engine's normalized document and checks nonempty cells, explicit reference columns and enums, source/readiness consistency, route/detail correspondence, prior-step references, ordered phase membership, and gate-terminated phase exits. Unknown producer-owned frontmatter keys remain valid. Plan-like text in explanatory prose is not a machine relationship. The semantic route audit must still reject:
